...
class MeshBinaryWriter(BinaryWriter):
    def __init__(self,fname,mesh_field_data):
        self.fname=fname
        self.mesh_field_data = np.array(mesh_field_data).reshape(-1).astype(np.float32)
        self.ncells = np.int32(mesh_field_data.size)
        self.size = np.array(np.shape(mesh_field_data),dtype=np.int32)
        self.shuffle_indices = None
    ...
    def write(self):
        with open(self.fname,'wb') as handle:
            byte_size = 0
            byte_size += self.write_header(handle)
            byte_size += self.write_vector_field(handle,self.mesh_field_data)
            return byte_size

    def write_header(self,handle=None):
        byte_size = 0
        header_size = 0

        ## size of the header
        header_size += 4 ## size of header
        header_size += 4 ## total number of cells
        header_size += 4*3 ## array of ints for dim sizes

        byte_size += self.write_int(handle,header_size)
        byte_size += self.write_int(handle,self.ncells)
        if handle is not None: 
            handle.write(self.size)
            byte_size += self.size.nbytes
...
if __name__ == '__main__':
    ...
    my_writer = MeshBinaryWriter('test.b',np.arange(1000).reshape(10,10,10))
    print(f'wrote {my_writer.write():d} bytes')